...
namespace Nivo4
{
    public partial class Lvl4 : Form
    {
        ...
        private List<Ptica> ptice = new List<Ptica>();
        ...
        public Lvl4()
        {
            this.Text = "Mačja Pustolovščina - Level 4";
        ...
        private void Lvl2_Paint(object sender, PaintEventArgs e)
        {
            ...
            foreach (var ptica in ptice)
                ptica.Narisi(g, premikKamere);
        ...
        private void Timer_Tick(object sender, EventArgs e)
        {
            ...
            {
                ptica.Posodobi();
                if (!gameOverTriggered && ptica.PreveriTrk(macka))
                {
                    gameOverTriggered = true;
                    GameOver();
                    return;
            ...
            int minY = (int)(this.ClientSize.Height * 0.2);
            int maxY = this.ClientSize.Height - 100;

            if (rnd.NextDouble() < 0.01 && ptice.Count < 5)
            {
                if (minY < maxY)
                {
                    int y = rnd.Next(minY, maxY);
                    ptice.Add(new Ptica(this.ClientSize.Width + premikKamere, y));
                }
            }
        ...
        private void ZakljuciStopnjo()
        {
            ...
            RazredBazaIgrice.PosodobiNivoInTocke(username, 4, macka.SteviloRibic);
            this.Close();
        }
        ...
        {
            MessageBox.Show("Muca je izgubila bitko s ptico!", "Game Over",
